...
int SaveEditor_SetKizunaMission(const char* path)
{
    ...
    // Find conditionFinishStates after the mission label
    char* label = strstr(buf, "\"DLC3_Main_Part4_KizunaProgress_Mission\"");
    if (!label) { free(buf); return -3; }
    char* cfs = strstr(label, "\"conditionFinishStates\"");
    if (!cfs) { free(buf); return -3; }
    char* arr_start = strchr(cfs, '[');
    if (!arr_start) { free(buf); return -3; }
    int depth = 1;
    ...
    memmove(arr_start + strlen("[true, true, true, true, true, true]"), arr_end, strlen(arr_end)+1);
    memcpy(arr_start, "[true, true, true, true, true, true]", strlen("[true, true, true, true, true, true]"));

    // Add to finishedMissions if not already there
    char* fm_label = strstr(buf, "\"DLC3_Main_Part4_KizunaProgress_Mission\"");
    if (!fm_label) { free(buf); return -3; }
    // Search for the label inside a string (skip the first match which is in allTrackingMissions)
    char* second = strstr(fm_label + 1, "\"DLC3_Main_Part4_KizunaProgress_Mission\"");
    if (!second)
    {
        // Not in finishedMissions yet, add it
        ...
        if (fin) {
            ...
            if (fb) {
                ...
                memmove(buf + pos + strlen(entry), buf + pos, strlen(buf + pos) + 1);
