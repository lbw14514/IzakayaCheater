...
东方夜雀食堂修改器 当前可以 加满好感 修改金钱 解锁全部地图 强开本体与 DLC1~DLC5 的 Boss 战
支持版本 鬼知道……反正支持 4.4.0
...
- 游戏素材著作权归游戏原作者所有 出处 [Steam - 东方夜雀食堂 - Touhou Mystia's Izakaya](https://store.steampowered.com/app/1584090/__Touhou_Mystias_Izakaya/)
- 若权利人认为本项目内容侵犯其权益 请通过 GitHub Issue 或邮件（3178644407@qq.com）联系 我会在收到通知后尽快删除相关内容（包括但不限于下架仓库 移除素材文件 更名） **侵权即删** 顺带提一嘴 我是一个学生 我如果没有及时回应 请耐心等待
...
> 3. 方案A「排队事件」**已停用（按钮置灰）** 只改存档 queue 实测触发不了 真正生效要在游戏运行时调用它自己的 `RunTimeScheduler.ScheduleEventExtern` 等于要改内存 注入
...
- Windows 10/11
- 需要 **WebView2 运行库** Win11 自带 Win10 装了新 Edge的话 也有
- 如果启动后窗口一片空白 装一下 [WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)
...
- 解锁全部地图
- 全部满好感
- 强开本体与 DLC1~DLC5 Boss 战

---

## 功能一 修改金钱

修改字段 `playerPartial.fund`

---

## 功能二 解锁全部地图

按钮在「金钱」下面 不依赖 Boss 选择
把 `dayScenePartial.daySceneMapStatusData` 与 `dayScenePartialDLC.DLCn.daySceneMapStatusData` 里的 17 张地图全部置 `true` 并补写 `allActivatedDLC` 的 DLC1~5

适用于只玩过本体的存档 地图上没有 DLC 地区 没法去打 DLC 再战的
...
重复点没事 第二次返回 0 且文件不变
注意 这只是「地图是否开放」 不改剧情进度 不等于你买了这个dlc

---

## 功能三 全部满好感

按钮在「解锁全部地图」右边 不依赖 Boss 选择
把 `albumPartial.specialSkinSelection` 与 `albumPartialDLC.<DLC>.specialSkinSelection` 里**所有角色**的

- `CurrentBondExp` 改成 `9999`
- `CurrentBondLevel` 改成 `5`

游戏满级就是 5 （

返回值

- `>=0` 改动的角色数（满进度档 78 个 纯本体档 29 个）
- `-4` 写入失败
- `-8` 该存档没有好感数据
...
| 方案B 标记通关 | 可用 | 把该战标记成已完成 解锁游戏里的「再战」入口 需要自己去对应 NPC 地点点一下 |
| 方案C 添加邀请函 | 可用 | 给物品 `2014~2019` 并给予好感 走原版路线（只有 DLC2 有） |
...
| 与幽幽子的决战 | B | 幽幽子羁绊特殊对话 |
| 饕餮挑战赛 | B | 妖怪山·荷取对话（山顶） |
| 怪诞料理挑战赛 | B | 地灵殿·阿燐对话（办公室门口右转） |
| 博丽大祭 | B | 博丽神社·时焉侑（进去右转） |
| 芙兰朵露挑战赛 | B | 红魔馆进去直走 右侧的门 → 地下室 → 芙兰的家 与芙兰对话 |
| 瑞灵 | B | 月都控制台（直走） |
...
**Q 改了没效果**
先确认已关闭 Steam 云同步 强开战斗方案 B 需要你自己去对应 NPC 地点点「再战」

**Q 会不会损坏存档**
任何修改都有风险 请先备份 重复点多数功能没事 但方案 B 可能因进度未到而丢失未完成进度

**Q 程序联网吗**
不联网 只读写本机存档 本地服务只监听 `127.0.0.1`
...
写 `schedulerPartialDLC.<DLC>.scheduledEvents` 键 = 日期 已停用
...
| 博丽大祭 | `DLC3_Repeat_GobackHakureiShrine_Event` | **方案 B** DLC3 羁绊 400 + 39 项开关（含 `DLC3_HakureiFestival_JienYuu`）+ 祭典任务 事件 | 博丽神社·时焉侑（进去右转） |
...
### 2.4 scheduledEvents 备注

`scheduledEvents` 的键是日期 自然存档里能看到

```json
"62": ["Main_5_BambooForest_001_Event"]
```

键 = 该事件要触发的那一天 已试过写 `-1` 桶和写次日的键 都触发不了 `ScheduleEventExtern` 最终进的是带一堆前置校验的运行时常驻入口 光改存档队列没用 要真正通过事件触发 只能在游戏运行时调用它 即注入 改内存
...
MSVC 就把 `-G` 换成对应生成器 并删掉那两个 wxWidgets 路径参数（让 CMake 自己找）

构建后 `build/` 里会有 `IzakayaCheater.exe` 同时自动拷入 `web/` 目录和 `WebView2Loader.dll`

分发时三者必须放一起 `IzakayaCheater.exe` `web/` `WebView2Loader.dll` 缺了界面会打不开

WebView2 的头文件与加载器已随仓库放在 `third_party/webview2` 取自 Microsoft.Web.WebView2 `1.0.4191.47`

---

## 6 RunTimeScheduler 映射
