...
| 方案 | 写入位置 | 效果 |
|---|---|---|
| 方案A 排队事件 | `scheduledEvents[第二天]`（键 = `playerPartial.gameDate.day` + 1）| 把事件排到第二天。读档后当天营业结束、推进到第二天时会直接进 Boss 战 |
...
> 注意：`scheduledEvents` 的键是**日期**（自然存档里能看到 `"62": ["Main_5_BambooForest_001_Event"]`，键 = 该事件要触发的那一天）。`-1` 是无日期桶，往那儿写不会自动执行。方案A 写的是「第二天」的键，所以用完之后要推进到下一天才会开打。
